...
class Full_VAE_Model(nn.Module):
    def __init__(self, input_dim, model_dim, tgt_size, n_declayers, d_ff, d_k_v, n_heads, latent_dim, dropout):
        ...
        self.decoder = DecoderVAE(tgt_size=tgt_size,
    ...
    def forward(self, rna_emds, rna_seq):
        ...
        z = mean + eps * std
        dec_outputs, _, _ = self.decoder(rna_seq, z)

        pred_seq = self.generator(dec_outputs)

        return pred_seq, mean, log_var


class EncoderVAE(nn.Module):
    def __init__(self, input_dim, model_dim, latent_dim, dropout):
...
class Generator(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        self.fc = nn.Linear(input_dim, vocab)
